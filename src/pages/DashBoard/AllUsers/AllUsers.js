...
const AllUsers = () => {
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://doctors-portal-server-sartajdev007.vercel.app/users');
    ...
    const handleMakeAdmin = id => {
        fetch(`https://doctors-portal-server-sartajdev007.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
